import s from './ApplicationProcessNavbar.module.scss';

const ApplicationProcessNavbar = ({ state = 1 }) => {
	return (
		<div className={s.main}>
			<div className={s.numbers}>
				...
				<div className={`${s.number} ${state >= 2 ? s.done : ''}`}>3</div>
				<div className={`${s.line} ${state >= 3 ? s.lineDone : ''}`} />
				<div className={`${s.number} ${state >= 3 ? s.done : ''}`}>4</div>
			</div>
			<div className={s.texts}>
				<span className={`${state >= 0 ? s.done : ''}`}>Review Bids</span>
...
export default ApplicationProcessNavbar;
